...
def get_kospi_tickers():
    ...
    for page in range(1, 21):  # 코스피 시가총액 상위 1000개 종목
...
# ----------------------------- 당일 종가 병합 -----------------------------
def get_today_price_df():
    today = datetime.today().strftime("%Y%m%d")
    try:
        prices = stock.get_market_ohlcv_by_ticker(today, market="ALL")[["종가"]]
        prices.reset_index(inplace=True)
        prices.columns = ["티커", today]  # <-- 컬럼명을 날짜 형식으로!
        return prices
    except Exception as e:
        print(f"당일 종가 데이터 수집 실패: {e}")
        return pd.DataFrame()
...
def main():
    ...
    # 최신 뉴스 컬럼 문자열화
    if '최신뉴스' in df.columns:
        df['최신뉴스'] = df['최신뉴스'].apply(
            lambda x: '\n'.join(x) if isinstance(x, list) else x
        )

    today_price_df = get_today_price_df()

    # 종가 병합
    merged_df = pd.merge(df, today_price_df, on="티커", how="left")
